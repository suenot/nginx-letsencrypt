...
program
  ...
  .option('-f, --file-name <nginx-file>', 'specify nginx file');
program.parse();
...
(async () => {
  try {
    ...
    await execCommand(`sudo touch /etc/nginx/sites-available/${fileName}`);
    await execCommand(`sudo chmod 777 /etc/nginx/sites-available/${fileName}`);

    fs.writeFileSync(`/etc/nginx/sites-available/${fileName}`, configWithoutCertificates(configurations));
    
    await execCommand(`sudo ln -sf /etc/nginx/sites-available/${fileName} /etc/nginx/sites-enabled/`);
    ...
    fs.writeFileSync(`/etc/nginx/sites-available/${fileName}`, configWithCertificates(configurations));
